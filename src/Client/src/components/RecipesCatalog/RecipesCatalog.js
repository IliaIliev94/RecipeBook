...
import { getRecipes, likeRecipe, unlikeRecipe, deleteRecipe } from "../../services/recipesService";
...
function RecipesCatalog({ searchParams }) {
    ...
    const { currentPage, buttonClickHandler, postsPerPage } = usePagination(6, searchParams);
    ...
    const likeHandler = async (recipeId, username) => {
        ...
        const recipe = recipesCopy.filter((recipe) => recipe.id === recipeId)[0];
    ...
    const unlikeHandler = async (recipeId, username) => {
        ...
        const recipe = recipesCopy.filter((recipe) => recipe.id === recipeId)[0];
        console.log(recipe);
        recipe.usersLiked = recipe.usersLiked?.filter((like) => like != username);
        setRecipes([...recipesCopy]);
    };
    ...
    const recipesToDisplay =
        searchParams == ""
            ...
            : recipes.filter(
                  (recipe) =>
                      recipe.title.toLowerCase().includes(searchParams.toLowerCase()) ||
                      recipe.description.toLowerCase().includes(searchParams.toLowerCase())
              );
